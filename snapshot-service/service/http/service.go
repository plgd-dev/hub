...
func New(serviceName string, config Config, snapshotServiceServer *grpcService.SnapshotServiceServer, validator *validator.Validator, fileWatcher *fsnotify.Watcher, logger log.Logger, tracerProvider trace.TracerProvider) (*Service, error) {
	...
	requestHandler, err := NewRequestHandler(&config, service.GetRouter(), snapshotServiceServer)
	if err != nil {
		_ = service.Close()
		return nil, err
	}
