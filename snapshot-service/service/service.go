...
import (
	...
	httpService "github.com/plgd-dev/hub/v2/snapshot-service/service/http"
...
func New(ctx context.Context, config Config, fileWatcher *fsnotify.Watcher, logger log.Logger) (*service.Service, error) {
	...
	ca, err := grpcService.NewSnapshotServiceServer(config.APIs.GRPC.Authorization.OwnerClaim, config.HubID, dbStorage, logger)
	if err != nil {
		closerFn.Execute()
