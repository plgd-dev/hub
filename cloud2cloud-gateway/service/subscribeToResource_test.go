...
func TestRequestHandlerSubscribeToResource(t *testing.T) {
	...
	_, shutdownDevSim := test.OnboardDevSim(ctx, t, c, deviceID, config.COAPS_TCP_SCHEME+testCfg.GW_HOST, test.GetAllBackendResourceLinks())
...
func TestRequestHandlerSubscribeToResourceTokenTimeout(t *testing.T) {
	...
	token := oauthTest.GetAccessToken(t, testCfg.OAUTH_SERVER_HOST, oauthTest.ClientTestShortExpiration)
	ctx = kitNetGrpc.CtxWithToken(ctx, token)

	_, shutdownDevSim := test.OnboardDevSimForClient(ctx, t, c, oauthTest.ClientTestShortExpiration, deviceID, testCfg.GW_HOST,
