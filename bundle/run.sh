...
# DIAL CERTS
export DIAL_CA_POOL="$CA_POOL_CERT_PATH"
export DIAL_CERT_DIR_PATH="$INTERNAL_CERT_DIR_PATH"
export DIAL_CERT_NAME="$GRPC_INTERNAL_CERT_NAME"
export DIAL_CERT_KEY_NAME="$GRPC_INTERNAL_CERT_KEY_NAME"

#LISTEN CERTS
export LISTEN_CA_POOL="$CA_POOL_CERT_PATH"
export LISTEN_CERT_DIR_PATH="$INTERNAL_CERT_DIR_PATH"
export LISTEN_CERT_NAME="$GRPC_INTERNAL_CERT_NAME"
export LISTEN_CERT_KEY_NAME="$GRPC_INTERNAL_CERT_KEY_NAME"
...
if [ "$INITIALIZE_CERITIFICATES" = "true" ]; then
  ...
  certificate-generator --cmd.generateCertificate --outCert=$DIAL_CERT_DIR_PATH/$DIAL_CERT_NAME --outKey=$DIAL_CERT_DIR_PATH/$DIAL_CERT_KEY_NAME --cert.subject.cn="localhost" --cert.san.domain="localhost" --signerCert=$CA_POOL_CERT_PATH --signerKey=$CA_POOL_CERT_KEY_PATH
...
# nats
nats-server --port $NATS_PORT --tls --tlsverify --tlscert=$DIAL_CERT_DIR_PATH/$DIAL_CERT_NAME --tlskey=$DIAL_CERT_DIR_PATH/$DIAL_CERT_KEY_NAME --tlscacert=$CA_POOL_CERT_PATH >$LOGS_PATH/nats-server.log 2>&1 &
status=$?
if [ $status -ne 0 ]; then
...
# mongo
cat $DIAL_CERT_DIR_PATH/$DIAL_CERT_NAME > $DIAL_CERT_DIR_PATH/mongo.key
cat $DIAL_CERT_DIR_PATH/$DIAL_CERT_KEY_NAME >> $DIAL_CERT_DIR_PATH/mongo.key
mongod --port $MONGO_PORT --dbpath $MONGO_PATH --sslMode requireSSL --sslCAFile $CA_POOL_CERT_PATH --sslPEMKeyFile $DIAL_CERT_DIR_PATH/mongo.key >$LOGS_PATH/mongod.log 2>&1 &
status=$?
if [ $status -ne 0 ]; then
...
LISTEN_CERT_DIR_PATH=${EXTERNAL_CERT_DIR_PATH} \
LISTEN_CERT_NAME=${COAP_GATEWAY_FILE_CERT_NAME} \
LISTEN_CERT_KEY_NAME=${COAP_GATEWAY_FILE_CERT_KEY_NAME} \
LISTEN_DISABLE_VERIFY_CLIENT_CERTIFICATE=${COAP_GATEWAY_DISABLE_VERIFY_CLIENTS} \
DISABLE_BLOCKWISE_TRANSFER=${COAP_GATEWAY_DISABLE_BLOCKWISE_TRANSFER} \
BLOCKWISE_TRANSFER_SZX=${COAP_GATEWAY_BLOCKWISE_TRANSFER_SZX} \
DISABLE_PEER_TCP_SIGNAL_MESSAGE_CSMS=${COAP_GATEWAY_DISABLE_PEER_TCP_SIGNAL_MESSAGE_CSMS} \
...
LOG_ENABLE_DEBUG=true \
LISTEN_DISABLE_VERIFY_CLIENT_CERTIFICATE=${GRPC_GATEWAY_DISABLE_VERIFY_CLIENTS} \
...
ADDRESS=${HTTP_GATEWAY_ADDRESS} \
LOG_ENABLE_DEBUG=true \
LISTEN_DISABLE_VERIFY_CLIENT_CERTIFICATE=${HTTP_GATEWAY_DISABLE_VERIFY_CLIENTS} \
...
LISTEN_DISABLE_VERIFY_CLIENT_CERTIFICATE=${CERTIFICATE_AUTHORITY_DISABLE_VERIFY_CLIENTS} \
