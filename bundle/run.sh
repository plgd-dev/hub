...
ADDRESS=${COAP_GATEWAY_UNSECURE_ADDRESS} LOG_ENABLE_DEBUG=true EXTERNAL_PORT=${COAP_GATEWAY_UNSECURE_PORT} FQDN=${COAP_GATEWAY_UNSECURE_FQDN} coap-gateway >$LOGS_PATH/coap-gateway-unsecure.log 2>&1 &
status=$?
if [ $status -ne 0 ]; then
...
ADDRESS=${GRPC_GATEWAY_ADDRESS} LISTEN_FILE_DISABLE_VERIFY_CLIENT_CERTIFICATE=${GRPC_GATEWAY_DISABLE_VERIFY_CLIENTS} grpc-gateway >$LOGS_PATH/grpc-gateway.log 2>&1 &
status=$?
if [ $status -ne 0 ]; then
