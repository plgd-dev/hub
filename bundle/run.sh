...
export SNAPSHOT_SERVICE_ADDRESS="localhost:${SNAPSHOT_SERVICE_PORT}"
export SNAPSHOT_SERVICE_HTTP_ADDRESS="localhost:${HTTP_SNAPSHOT_SERVICE_PORT}"
...
if [ "${OVERRIDE_FILES}" = "true" ] || [ ! -f "${NGINX_PATH}/nginx.conf" ]; then
  ...
  sed -i "s/REPLACE_HTTP_SNAPSHOT_SERVICE_PORT/$HTTP_SNAPSHOT_SERVICE_PORT/g" ${NGINX_PATH}/nginx.conf
fi
...
## configuration
if [ "${OVERRIDE_FILES}" = "true" ] || [ ! -f "/data/snapshot-service.yaml" ]; then
cat /configs/snapshot-service.yaml | yq e "\
  .hubID = \"${HUB_ID}\" |
  .log.level = \"${LOG_LEVEL}\" |
  .apis.grpc.address = \"${SNAPSHOT_SERVICE_ADDRESS}\" |
  .apis.grpc.authorization.audience = \"${SERVICE_OAUTH_AUDIENCE}\" |
  .apis.grpc.authorization.http.tls.useSystemCAPool = true |
  .apis.grpc.authorization.authority = \"https://${OAUTH_ENDPOINT}\" |
  .apis.grpc.authorization.ownerClaim = \"${OWNER_CLAIM}\" |
  .apis.http.address = \"${SNAPSHOT_SERVICE_HTTP_ADDRESS}\" |
  .clients.storage.use = \"${DATABASE_USE}\" |
  .clients.storage.mongoDB.uri = \"${MONGODB_URI}\" |
  .clients.storage.cqlDB.hosts = [ \"${SCYLLA_HOSTNAME}\" ] |
  .clients.storage.cqlDB.port = ${SCYLLA_PORT} |
  ...
  .clients.openTelemetryCollector.grpc.tls.certFile = \"${OPEN_TELEMETRY_EXPORTER_CERT_FILE}\" |
  .clients.openTelemetryCollector.grpc.tls.useSystemCAPool = true
" - > /data/snapshot-service.yaml
fi
snapshot-service --config /data/snapshot-service.yaml >$LOGS_PATH/snapshot-service.log 2>&1 &
status=$?
certificate_authority_pid=$!
if [ $status -ne 0 ]; then
  echo "Failed to start snapshot-service: $status"
  sync
  cat $LOGS_PATH/snapshot-service.log
  exit $status
fi
...
while true; do
  ...
  echo "Try to reconnect to snapshot-service(${CERTIFICATE_AUTHORITY_ADDRESS}) $i"
  sleep 1
done
...
while sleep 10; do
  ...
  ps aux |grep $snapshot_service_pid |grep -q -v grep
  if [ $? -ne 0 ]; then
    echo "snapshot-service has already exited."
    sync
    cat $LOGS_PATH/snapshot-service.log
   exit 1
  fi
