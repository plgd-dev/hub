...
func New(serviceName string, config Config, integrationServiceServer *grpcService.IntegrationServiceServer, validator *validator.Validator, fileWatcher *fsnotify.Watcher,
	logger log.Logger, tracerProvider trace.TracerProvider) (*Service, error) {
	...
	requestHandler, err := NewRequestHandler(&config, service.GetRouter(), integrationServiceServer)
	if err != nil {
		_ = service.Close()
		return nil, err
	}
