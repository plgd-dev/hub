...
test('devices-list-open-detail', async ({ page }) => {
    ...
    const deviceId = await page.getByTestId(`${testId.devices.detail.informationTableId}-value`).textContent()

    await expect(page).toHaveURL(`${urlBase}/devices/${deviceId}`)
    ...
    await expect(page).toHaveURL(`${urlBase}/devices/${deviceId}/resources`)
})
