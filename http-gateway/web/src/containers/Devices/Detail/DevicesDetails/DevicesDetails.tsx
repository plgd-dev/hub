...
const DevicesDetails: FC<Props> = memo(({ data, isTwinEnabled, loading, setTwinSynchronization, twinSyncLoading }) => {
        ...
        <Row>
            <Col>
                ...
                <LabelWithLoading title={_(t.types)}>
                    <div className='align-items-end badges-box-vertical'>{data?.types?.map?.((type: string) => <Badge key={type}>{type}</Badge>) || '-'}</div>
                </LabelWithLoading>
            </Col>
            <Col>
                <LabelWithLoading title={_(t.status)}>
                    <Badge className={isOnline ? 'green' : 'red'}>{isOnline ? _(t.online) : _(t.offline)}</Badge>
                </LabelWithLoading>
