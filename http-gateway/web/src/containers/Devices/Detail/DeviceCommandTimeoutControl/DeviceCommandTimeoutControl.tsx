...
import { convertValueToNs, hasCommandTimeoutError, convertAndNormalizeValueFromTo, normalizeToFixedFloatValue, findClosestUnit } from '../../utils'
...
const DeviceCommandTimeoutControl: FC<Props> = (props) => {
    const { defaultValue, defaultTtlValue, onChange, disabled, ttlHasError, onTtlHasError, isDelete } = { ...defaultProps, ...props }
    ...
    return (
        ...
        >
            <div className='ttl-label-content d-flex align-items-center justify-content-end'>
                {unit !== INFINITE ? (
                    <TextField
                        ...
                        placeholder={`${_(t.default)} (${closestDefaultTtl.value}${closestDefaultTtl.unit})`}
                        value={inputValue}
                    />
                ) : (
            ...
            {ttlHasError && <div className='error-message'>{_(t.minimalValueIs, { minimalValue: '100ms' })}</div>}
        </Label>
    )
}
