...
func (requestHandler *RequestHandler) ServeWs(w http.ResponseWriter, r *http.Request, ob ObservationResolver) error {
	...
	c.SetReadLimit(requestHandler.config.Service.Http.Capabilities.WebSocketReadLimit)
	c.SetReadDeadline(time.Now().Add(time.Second * requestHandler.config.Service.Http.Capabilities.WebSocketReadTimeout))
