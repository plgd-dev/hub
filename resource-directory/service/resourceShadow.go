...
func (rd *ResourceTwin) filterResources(resourceIDsFilter []*commands.ResourceId, typeFilter []string, includeNonexistentResources bool, toReloadDevices strings.Set, onResource func(*Resource) error) error {
	mapTypeFilter := make(strings.Set)
	mapTypeFilter.Add(typeFilter...)
	return rd.projection.LoadResources(resourceIDsFilter, mapTypeFilter, includeNonexistentResources, toReloadDevices, onResource)
}
...
func (rd *ResourceTwin) sendPendingCommands(srv pb.GrpcGateway_GetPendingCommandsServer, resourceIDsFilter []*pb.ResourceIdFilter, typeFilter []string, filterCmds subscription.FilterBitmask, includeNonexistentResources bool, now time.Time, toReloadDevices strings.Set) error {
	return rd.filterResources(resourceIdFilterToSimple(resourceIDsFilter), typeFilter, includeNonexistentResources, toReloadDevices, func(resource *Resource) error {
...
func (rd *ResourceTwin) GetPendingCommands(req *pb.GetPendingCommandsRequest, srv pb.GrpcGateway_GetPendingCommandsServer) error {
	...
	resourceIDsFilter := rd.convertToResourceIDs(req.GetResourceIdFilter(), req.GetDeviceIdFilter())
	toReloadDevices := make(strings.Set)
	err = rd.sendPendingCommands(srv, resourceIDsFilter, req.GetTypeFilter(), filterCmds, req.GetIncludeNonexistentResources(), now, toReloadDevices)
	if err != nil {
		return err
	}
	if len(toReloadDevices) > 0 {
		...
		return rd.sendPendingCommands(srv, newResourceIDsFilter, req.GetTypeFilter(), filterCmds, req.GetIncludeNonexistentResources(), now, nil)
	}
	return nil
}
