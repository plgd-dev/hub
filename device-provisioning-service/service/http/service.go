...
func New(ctx context.Context, serviceName string, config Config, fileWatcher *fsnotify.Watcher, logger log.Logger, tracerProvider trace.TracerProvider, store store.Store) (*Service, error) {
	...
	auth := kitNetHttp.NewInterceptorWithValidator(validator, kitNetHttp.NewDefaultAuthorizationRules(APIV1))
	mux := serverMux.New()
	...
	httpServer := &http.Server{
		Handler:           kitNetHttp.OpenTelemetryNewHandler(r, serviceName, tracerProvider),
