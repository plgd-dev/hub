...
func TestResourceDirectoryDeleteHandler(t *testing.T) {
	...
	deletetblResourceDirectory := []testEl{
		{"NotExist1", input{coapCodes.DELETE, ``, []string{"di=c", "ins=5"}}, output{coapCodes.BadRequest, `cannot get observed resources: invalid deviceID`, nil}, true},            // Non-existent device ID.
