...
func TestRefreshTokenPostHandlerVerifyToken(t *testing.T) {
	...
	tests := []test{
		...
		{
			name: "valid - userID from request",
			args: args{
				accessToken: security.CreateJwksToken(t, jwt.MapClaims{
					oauthUri.DeviceIDClaimKey: deviceID,
				}),
	...
	for _, tt := range tests {
		tf := func(t *testing.T) {
			deviceID, owner, err := getRefreshTokenDataFromClaims(context.Background(), client, tt.args.accessToken, tt.args.req)
			if tt.wantErr == nil {
