...
func refreshTokenPostHandler(s mux.ResponseWriter, req *mux.Message, client *Client) {
	...
	if err != nil {
		logAndWriteErrorResponse(fmt.Errorf("cannot handle refresh token: %v", err), client, coapCodes.BadRequest, req.Token)
		return
	}
	...
	if err != nil {
		logAndWriteErrorResponse(fmt.Errorf("cannot handle refresh token: %v", err), client, coapconv.GrpcCode2CoapCode(status.Convert(err).Code(), coapCodes.POST), req.Token)
		return
	}
