...
func startResourceObservation(req *mux.Message, client *Client, authCtx *authorizationContext, deviceID, href string) {
	...
	for {
		...
		if err != nil {
			client.logAndWriteErrorResponse(fmt.Errorf("DeviceId: %v: cannot start resource observation /%v%v: %w", authCtx.GetDeviceID(), deviceID, href, err), coapconv.GrpcCode2CoapCode(status.Convert(err).Code(), coapconv.Retrieve_Operation), req.Token)
			return
		}
	...
	h := resourceSubscriptionHandlers{
		onChange: func(ctx context.Context, resourceChanged *pb.Event_ResourceChanged) error {
			if resourceChanged.GetStatus() != pbGRPC.Status_OK {
				...
				client.logAndWriteErrorResponse(fmt.Errorf("DeviceId: %v: cannot observe resource /%v%v, device response: %v", authCtx.GetDeviceID(), deviceID, href, resourceChanged.GetStatus()), coapconv.StatusToCoapCode(resourceChanged.GetStatus(), coapconv.Retrieve_Operation), req.Token)
				return nil
			}
