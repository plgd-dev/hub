...
func validateUpdateDeviceMetadata(request *commands.UpdateDeviceMetadataRequest) error {
	...
	if request.GetStatus() == nil && request.GetShadowSynchronizationStatus() == nil && request.GetShadowSynchronization() == commands.ShadowSynchronization_UNSET {
		return status.Errorf(codes.InvalidArgument, "set.onlineStatus, set.shadowSynchronization, shadowSynchronizationStatus are invalid")
	}
