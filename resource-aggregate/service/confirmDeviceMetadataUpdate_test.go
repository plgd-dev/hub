...
func TestAggregateHandleConfirmDeviceMetadataUpdate(t *testing.T) {
	...
	ag, err := service.NewAggregate(commands.NewResourceID(deviceID, commands.StatusHref), 10, cfg.Clients.Eventstore.HubID, eventstore, service.DeviceMetadataFactoryModel, cqrsAggregate.NewDefaultRetryFunc(1))
	require.NoError(t, err)
	...
	for _, tt := range test {
		tfunc := func(t *testing.T) {
			ag, err := service.NewAggregate(commands.NewResourceID(tt.args.request.GetDeviceId(), commands.StatusHref), 10, cfg.Clients.Eventstore.HubID, eventstore, service.DeviceMetadataFactoryModel, cqrsAggregate.NewDefaultRetryFunc(1))
			require.NoError(t, err)
