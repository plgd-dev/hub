...
func TestAggregateHandle_UpdateDeviceMetadata(t *testing.T) {
	...
	test := []struct {
...

		{
			name: "set online",
			args: args{
				request: testMakeUpdateDeviceMetadataRequest("dev0", newBool(true), nil),
				userID:  "user0",
			},
			want: codes.OK,
		},
...
func TestRequestHandler_UpdateDeviceMetadata(t *testing.T) {
	...
	test := []struct {
	...
	}{
		{
			name: "set online",
			args: args{
				request: testMakeUpdateDeviceMetadataRequest(deviceID, newBool(true), nil),
			},
			want: &commands.UpdateDeviceMetadataResponse{
...
func testMakeUpdateDeviceMetadataRequest(deviceID string, online *bool, shadowSynchronizationDisabled *bool) *commands.UpdateDeviceMetadataRequest {
	...
	if online != nil {
		r.Update = &commands.UpdateDeviceMetadataRequest_Online{
			Online: &commands.OnlineStatus{
				Value: *online,
			},
		}
	}
